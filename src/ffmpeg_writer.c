...
ffmpeg_writer *ffmpeg_writer_create(const char *filename, int width, int height, int fps) {
    ...
    if (avcodec_open2(w->codec_ctx, codec, NULL) < 0) return NULL;

    if (avcodec_parameters_from_context(w->stream->codecpar, w->codec_ctx) < 0) return NULL;
    ...
    if (av_frame_get_buffer(w->frame, 32) < 0) return NULL;

    w->sws_ctx = sws_getContext(width, height, AV_PIX_FMT_RGB24,
                                width, height, AV_PIX_FMT_YUV420P,
                                SWS_BICUBIC, NULL, NULL, NULL);
    if (!w->sws_ctx) return NULL;
...
void ffmpeg_writer_add_frame(ffmpeg_writer *w, uint8_t *rgb_data) {
    ...
    sws_scale(w->sws_ctx, src_slice, src_stride, 0, w->height,
