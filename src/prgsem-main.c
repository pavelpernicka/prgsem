...
static struct argp_option options[] = {
    ...
    {"output", 1004, "FILE", 0,
     "Output image file (.png or .jpg for images, .mp4 for videos)"},
    {"anim-fps", 1005, "N", 0, "Number of frames per second in video"
    }, // only if cli, >0, <120
    {"anim-duration", 1007, "N", 0, "Duration of animation in seconds"
    }, // only if cli, >0, <36000
    {"anim-zoom", 1006, "F", 0,
     "Zoom factor from start to the end of video sequence (e.g. 0.5)"}, // only
                                                                        // if
                                                                        // cli,
                                                                        // > 0,
                                                                        // <=
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
  ...
  case 'w':
    ...
    if (args->w < 50 || args->w > 10000 || args->w % CHUNK_SIZE_FACTOR != 0) {
      argp_error(
          state, "Invalid width (must be 50–10000 and divisible by %d)",
          CHUNK_SIZE_FACTOR
      );
    }
    break;
  case 'h':
    ...
    if (args->h < 50 || args->h > 10000 || args->h % CHUNK_SIZE_FACTOR != 0) {
      argp_error(
          state, "Invalid height (must be 50–10000 and divisible by %d)",
          CHUNK_SIZE_FACTOR
      );
    }
    break;
...
bool apply_args_to_ctx(struct arguments *args, comp_ctx *ctx) {
  if (args->w <= 0 || args->h <= 0 || args->n <= 0) {
    error(
        "Invalid size or iteration count: w=%d, h=%d, n=%d", args->w, args->h,
        args->n
    );
    return EXIT_ERROR;
  }
  if (args->range_re_min >= args->range_re_max ||
    ...
    error(
        "Invalid coordinate range: re_min=%.3f, re_max=%.3f, "
        "im_min=%.3f, im_max=%.3f",
...
bool module_handshake(app_state *state) {
  ...
  while (!is_quit()) {
	...
	info(
	    "Handshake successful, module version matching: %d.%d.%d",
	    module.major, module.minor, module.patch
	);
      }
      free(msg);
...
void process_event(app_state *state, event *ev) {
  if (ev->type == EV_QUIT) {
    ...
    case MSG_VERSION:
      info(
          "Module version: %d.%d.%d", msg->data.version.major,
...
void local_compute(app_state *state) {
  ...
  for (int y = 0; y < h; ++y) {
    for (int x = 0; x < w; ++x) {
      ...
      grid[y * w + x] = compute_pixel(
          state->ctx->c_re, state->ctx->c_im, z_re, z_im, state->ctx->n
      );
    }
  }
