...
int xwin_init(int w, int h) {
  ...
  win = SDL_CreateWindow(
      "PRG Semester Project", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
...
void xwin_set_event_pusher(event_pusher_fn handler) { event_push = handler; }

int is_valid_sdl_key(SDL_Keycode key) {
  return (key >= 'a' && key <= 'z') || (key >= 'A' && key <= 'Z') ||
         (key >= '0' && key <= '9') || key == TAB || key == F1 ||
         (key >= RIGHT && key <= FRONT);
}
...
bool show_helpscreen(int w, int h) {
  ...
  int y = 30;
  for (int i = 0; i < (int)(sizeof(showhelp_lines) / sizeof(showhelp_lines[0]));
       ++i) {
    SDL_Surface *text =
        TTF_RenderUTF8_Blended(font, showhelp_lines[i], textColor);
    if (!text)
      continue;

    SDL_Rect dest = {
        .x = (w - text->w) / 2, .y = y, .w = text->w, .h = text->h
    };
...
void xwin_draw_overlay_message(SDL_Surface *surf) {
  ...
  SDL_Rect dest = {
      .x = surf->w - text->w - 10, .y = 10, .w = text->w, .h = text->h
  };
...
void *window_thread(void *arg) {
  ...
  while (!is_quit()) {
	...
	if (is_valid_sdl_key(key)) {
	  event ev = {
	      .source = EV_SDL, .type = EV_KEYBOARD, .data.param = (int)key
	  };
	  event_push(ev);
	}
	break;
